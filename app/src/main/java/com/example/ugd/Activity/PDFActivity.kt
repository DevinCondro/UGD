...
class PDFActivity : AppCompatActivity() {
        ...
        binding!!.buttonSave.setOnClickListener {
            ...
            try {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O){
                    if(nama.isEmpty() && email.isEmpty() && umur.isEmpty() && tlp.isEmpty() && alamat.isEmpty()){
                        Toast.makeText(applicationContext, "Semuanya Tidak Boleh Kosong", Toast.LENGTH_SHORT).show()
                    }else{
                        createPdf(nama, email, umur, tlp, alamat)
                    }
                }
    ...
    private fun createPdf(nama: String, email: String, umur: String, tlp: String, alamat: String) {
        val pdfPath = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).toString()
        val file = File(pdfPath, "pdf_10729.pdf")
        ...
        table.addCell(Cell().add(Paragraph("Nama User")))
        ...
        table.addCell(Cell().add(Paragraph("Alamat User")))
